...
void 
insert(char *name, Type_Expression type, int size, int offset) {
  int currentIndex;
  int visitedSlots = 0;
  ...
  HashTable[currentIndex]->size= size; /* in bytes */
}
...
PrintSymbolTable() {
  ...
  for (i=0; i < HASH_TABLE_SIZE; i++) {
    if (HashTable[i] != NULL) {
      printf("\t \"%s\" of type %s with size %d with offset %d\n", 
		HashTable[i]->name, TypeToString(HashTable[i]->type), HashTable[i]->size,  HashTable[i]->offset); 
    }
  }
  printf("\n --------------------------------\n\n");
}
